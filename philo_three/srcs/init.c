...
int		init_setup(t_setup *setup, int ac, char **av)
{
	...
	sem_unlink("writing");
	if ((setup->forks =
	sem_open("Forks", O_CREAT | O_EXCL, 0644, setup->philo_num)) == SEM_FAILED)
		return (1);
	if ((setup->is_dead =
		sem_open("is_dead", O_CREAT | O_EXCL, 0644, 1)) == SEM_FAILED)
		return (1);
	if ((setup->writing =
		sem_open("writing", O_CREAT | O_EXCL, 0644, 1)) == SEM_FAILED)
		return (1);
